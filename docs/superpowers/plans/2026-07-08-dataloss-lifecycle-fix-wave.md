...
9. **Importance dual-write divergence (Task 6 review).** Rebalance writes `importance` both to the store column and into `metadata.importance`; `decay-engine.ts` reads the metadata copy. A future column-only update (pin, access-tracker) leaves the metadata copy stale. Harmless today; document the column as authoritative or stop persisting importance into metadata.
